...
## Hero scenario
1) Check out `duracell04/goni` at the full revision pinned in
   `canonical-basis.json`, then ingest Markdown from its `knowledge/` directory
   into Qdrant or a mock batch.
...
# 2) Ingest a separately checked-out, full-SHA-pinned canonical corpus
python prototype/01-rag-slice/ingest.py --src ../goni/knowledge \
...
- Source boundary: do not ingest a mutable branch as canonical input. Record the
  full `goni` SHA with any result submitted as evidence.
